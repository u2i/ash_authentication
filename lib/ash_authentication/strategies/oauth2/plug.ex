...
defmodule AshAuthentication.Strategy.OAuth2.Plug do
  ...
  #
  # 2. An IdP/third-party-initiated login (e.g. a Clever or ClassLink portal
  #    launch): the provider redirects straight to our callback with a `code`
  #    but no `state`, so the request phase never ran and nothing was stored.
  #    We must NOT complete authentication from this request — with no stored
  #    `state` there is no CSRF binding to verify, and "session absent" is an
  #    attacker-controllable condition (a normal flow with the cookie stripped
  #    is indistinguishable), so trusting it would disable `state` verification
  #    for every flow on the strategy. Instead, when the strategy opts in via
  #    `idp_initiated_login?`, we treat the callback as a *trigger* and restart
  #    the request phase, which mints a fresh `state` we later verify — the
  #    OpenID Connect Core §4 ("Initiating Login from a Third Party") pattern,
  #    and the handling Clever's own docs prescribe. The user still has a live
  #    session at the provider, so the restarted flow returns immediately. The
  #    restart stores a session, so its callback takes the success path above
  #    and never re-enters here — the bounce happens at most once.
  defp maybe_reflect_or_fail(conn, strategy) do
    cond do
      conn.method == "POST" and not reflected?(conn) ->
        render_interstitial(conn, strategy)

      conn.method == "GET" and Map.get(strategy, :idp_initiated_login?, false) ->
